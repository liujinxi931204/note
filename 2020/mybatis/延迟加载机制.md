...
可以看到主查询`select id,name from category`被执行，关联查询`select id,name,price,cid from product where cid=#{cid}`同时也被执行了，就是同时设置lazyLoadingEnabled和aggressiveLazyLoading为true时会
