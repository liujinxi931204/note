...
可以看到主查询`select id,name from category`和关联查询`select id,name,price,cid from product where cid=#{cid}`都被执行了，说明fetchType这个标签覆盖了核心配置文件中`aggressiveLazyLoading=false`
