...
`--listen-addr<format: <ip|interface>[:port]>`指定用于承载swarm流量的IP和端口，其设置通常与`--advetise-addr`相匹配，有多个节点的时候可以指定使用哪个IP  
...
#### 接入工作节点  
`docker swarm join --token SWMTKN-1-0uahebax...c87tu8dx2c 10.0.0.1:2377 --ad`
