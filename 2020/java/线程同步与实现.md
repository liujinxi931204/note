...
+ ReentrantLock.lockInterruptibly允许在等待时由其他线程调用等待线程的interrupt()方法来中断等待线程而直接返回，这时不用获取锁，而会抛出一个InterruptedExection异常。ReentrantLock.lock方法不允许Thread.interrupt中断，即使检测到Thread.isInterrupted,一样会继续尝试获取锁，失败则继续休眠。只是在最后获取锁成功后再把
