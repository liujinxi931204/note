...
thread2.start();
```  
这时创建了两个threadSecond对象threadSecond1和threadSecond2，线程thread1执行的是threadSecond1对象中的synchornized代码(run)，线程thread2执行的是threadSecond2对象中的synchornized代码(run)。因为synchornized锁定的是对象，这时会有两把锁分别锁定threadSecond1对象和threadSecond2对象，而这两把锁相互之间不干扰，
