...
注意：**UDP程序在receive()方法出阻塞，直到收到一个数据报文或等待超时。由于UDP协议是不可靠协议，如果数据报文在传输过程中发生丢失，那么程序将会一直阻塞在receive()方法处，这样客户端将永远都接收不到服务端发送回来的数据，但是又没有任何提时。因此，在客户端使用DatagramSocket类的setSoTimeout()方法来制定** 
