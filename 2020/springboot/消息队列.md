...
```  
结果如下  
![title](https://raw.githubusercontent.com/liujinxi931204/image/master/gitnote/2020/12/23/1608717620076-1608717620132.png)  
可以看到获取了消息体、消息头等内容  
...
```
结果如下  
![title](https://raw.githubusercontent.com/liujinxi931204/image/master/gitnote/2020/12/23/1608722072377-1608722072378.png)  
...
结果如下  
![title](https://raw.githubusercontent.com/liujinxi931204/image/master/gitnote/2020/12/23/1608722814635-1608722814636.png)  
如果@RabbitHandler注解的方法中没有一个的类型可以和消息类型对的上，比如消息都是byte数组，这里却没有对应的方法接收，系统就会在控制台不停的报错，如果出现这个情况说明类型写的不正确  
比如队列中会出现byte、String和序列化这三种类型，那就必须得有对应的处理这三种消息的方法，不然消息发送过来会因为无法正确的类型转换而报错  
而且使用了@RabbitHandler注解之后就不能再使用Message作为接收类型了
